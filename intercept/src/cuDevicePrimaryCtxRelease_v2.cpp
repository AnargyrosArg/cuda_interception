...
#include <dlfcn.h>
#include <iostream>
CUresult (*original_cuDevicePrimaryCtxRelease_v2)(
CUdevice
);
...
extern "C"
{
	CUresult cuDevicePrimaryCtxRelease_v2(CUdevice dev) {
		...
		if (!original_cuDevicePrimaryCtxRelease_v2)
		{
			...
			original_cuDevicePrimaryCtxRelease_v2 = (CUresult (*)(
			CUdevice)
			) dlsym(original_libcuda_handle, "cuDevicePrimaryCtxRelease_v2");
			fprintf(stderr, "original_cuDevicePrimaryCtxRelease_v2:%p\n", original_cuDevicePrimaryCtxRelease_v2);
		}
		__dlerror = dlerror();
		if(__dlerror){
