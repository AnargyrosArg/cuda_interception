...
#include <dlfcn.h>
#include <iostream>
nvrtcResult (*original_nvrtcGetNVVMSize)(
nvrtcProgram, 
size_t *
);
...
extern "C"
{
	nvrtcResult nvrtcGetNVVMSize(nvrtcProgram prog, size_t *nvvmSizeRet) {
		fprintf(stderr, "nvrtcGetNVVMSize()\n");
		char* __dlerror;
		...
		if (!original_nvrtcGetNVVMSize)
		{
			//fetch the original function addr using dlsym
			original_nvrtcGetNVVMSize = (nvrtcResult (*)(
			nvrtcProgram, 
			size_t *)
			) dlsym(original_libnvrtc_handle, "nvrtcGetNVVMSize");
		}
		__dlerror = dlerror();
		if(__dlerror){
			fprintf(stderr, "dlsym error for function nvrtcGetNVVMSize():%s\n", __dlerror);
			fflush(stderr);
		}
		return original_nvrtcGetNVVMSize(
		prog, 
		nvvmSizeRet
		);
	}
