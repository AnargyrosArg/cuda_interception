#include <nvrtc.h>
#include <dlfcn.h>
#include <iostream>
nvrtcResult (*original_nvrtcGetPTX)(
nvrtcProgram, 
char *
);
//handle to the actual libnvrtc library, used to fetch original functions with dlsym
extern void* original_libnvrtc_handle;
extern "C"
{
	nvrtcResult nvrtcGetPTX(nvrtcProgram prog, char *ptx) {
		fprintf(stderr, "nvrtcGetPTX()\n");
		char* __dlerror;
		//this call clears any previous errors
		dlerror();
		//this calls the intercepted dlopen function which initializes the handle to libnvrtc (original_libnvrtc_handle)
		if(original_libnvrtc_handle == NULL){
			dlopen("libnvrtc.so.11.2", RTLD_NOW);
		}
		if (!original_nvrtcGetPTX)
		{
			//fetch the original function addr using dlsym
			original_nvrtcGetPTX = (nvrtcResult (*)(
			nvrtcProgram, 
			char *)
			) dlsym(original_libnvrtc_handle, "nvrtcGetPTX");
		}
		__dlerror = dlerror();
		if(__dlerror){
			fprintf(stderr, "dlsym error for function nvrtcGetPTX():%s\n", __dlerror);
			fflush(stderr);
		}
		return original_nvrtcGetPTX(
		prog, 
		ptx
		);
	}
}